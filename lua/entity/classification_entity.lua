-- Openfda SDK Classification entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ClassificationEntity = {}
ClassificationEntity.__index = ClassificationEntity


function ClassificationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ClassificationEntity)
...
function ClassificationEntity:get_name()
  return self._name
end


function ClassificationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ClassificationEntity.new(self._client, opts)
end
...
function ClassificationEntity:mark_deleted()
  self._deleted = true
end


function ClassificationEntity:deleted()
  return true == self._deleted
end


function ClassificationEntity:data_set(args)
  if args ~= nil then
...
function ClassificationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ClassificationEntity:match_set(args)
  if args ~= nil then
...
function ClassificationEntity:match_get()
...
function ClassificationEntity:stream(action, args, callopts)
...
---@param reqmatch ClassificationListMatch
---@param ctrl? table
---@return Classification[]
---@return string? err
function ClassificationEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ClassificationEntity:_run_op(ctx, post_done)
  local utility = self._utility
