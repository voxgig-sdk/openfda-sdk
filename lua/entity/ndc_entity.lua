...
local NdcEntity = {}
NdcEntity.__index = NdcEntity


function NdcEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NdcEntity)
  self._name = "ndc"
  self._client = client
...
function NdcEntity:get_name()
  return self._name
end


function NdcEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NdcEntity.new(self._client, opts)
end
...
function NdcEntity:mark_deleted()
  self._deleted = true
end


function NdcEntity:deleted()
  return true == self._deleted
end


function NdcEntity:data_set(args)
  if args ~= nil then
...
function NdcEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NdcEntity:match_set(args)
  if args ~= nil then
...
function NdcEntity:stream(action, args, callopts)
...
function NdcEntity:_run_op(ctx, post_done)
  local utility = self._utility
