...
local NsdeEntity = {}
NsdeEntity.__index = NsdeEntity


function NsdeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NsdeEntity)
  self._name = "nsde"
  self._client = client
...
function NsdeEntity:get_name()
  return self._name
end


function NsdeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NsdeEntity.new(self._client, opts)
end
...
function NsdeEntity:mark_deleted()
  self._deleted = true
end


function NsdeEntity:deleted()
  return true == self._deleted
end


function NsdeEntity:data_set(args)
  if args ~= nil then
...
function NsdeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NsdeEntity:match_set(args)
  if args ~= nil then
...
function NsdeEntity:stream(action, args, callopts)
...
function NsdeEntity:_run_op(ctx, post_done)
  local utility = self._utility
