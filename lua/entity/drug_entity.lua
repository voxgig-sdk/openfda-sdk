...
local DrugEntity = {}
DrugEntity.__index = DrugEntity


function DrugEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DrugEntity)
  self._name = "drug"
  self._client = client
...
function DrugEntity:get_name()
  return self._name
end


function DrugEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DrugEntity.new(self._client, opts)
end
...
function DrugEntity:mark_deleted()
  self._deleted = true
end


function DrugEntity:deleted()
  return true == self._deleted
end


function DrugEntity:data_set(args)
  if args ~= nil then
...
function DrugEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DrugEntity:match_set(args)
  if args ~= nil then
...
function DrugEntity:stream(action, args, callopts)
...
---@param reqmatch DrugListMatch
---@param ctrl? table
---@return Drug[]
---@return string? err
function DrugEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function DrugEntity:_run_op(ctx, post_done)
  local utility = self._utility
