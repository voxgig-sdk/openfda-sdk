...
local ShortageEntity = {}
ShortageEntity.__index = ShortageEntity


function ShortageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ShortageEntity)
  self._name = "shortage"
  self._client = client
...
function ShortageEntity:get_name()
  return self._name
end


function ShortageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ShortageEntity.new(self._client, opts)
end
...
function ShortageEntity:mark_deleted()
  self._deleted = true
end


function ShortageEntity:deleted()
  return true == self._deleted
end


function ShortageEntity:data_set(args)
  if args ~= nil then
...
function ShortageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ShortageEntity:match_set(args)
  if args ~= nil then
...
function ShortageEntity:match_get()
...
function ShortageEntity:stream(action, args, callopts)
...
function ShortageEntity:_run_op(ctx, post_done)
  local utility = self._utility
