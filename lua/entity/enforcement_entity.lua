...
local EnforcementEntity = {}
EnforcementEntity.__index = EnforcementEntity


function EnforcementEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EnforcementEntity)
  self._name = "enforcement"
  self._client = client
...
function EnforcementEntity:get_name()
  return self._name
end


function EnforcementEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EnforcementEntity.new(self._client, opts)
end
...
function EnforcementEntity:mark_deleted()
  self._deleted = true
end


function EnforcementEntity:deleted()
  return true == self._deleted
end


function EnforcementEntity:data_set(args)
  if args ~= nil then
...
function EnforcementEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EnforcementEntity:match_set(args)
  if args ~= nil then
...
function EnforcementEntity:match_get()
...
function EnforcementEntity:stream(action, args, callopts)
...
function EnforcementEntity:_run_op(ctx, post_done)
  local utility = self._utility
