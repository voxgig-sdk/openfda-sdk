-- Openfda SDK N510k entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local N510kEntity = {}
N510kEntity.__index = N510kEntity


function N510kEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, N510kEntity)
  self._name = "n510k"
  self._client = client
...
function N510kEntity:get_name()
  return self._name
end


function N510kEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return N510kEntity.new(self._client, opts)
end
...
function N510kEntity:mark_deleted()
  self._deleted = true
end


function N510kEntity:deleted()
  return true == self._deleted
end


function N510kEntity:data_set(args)
  if args ~= nil then
...
function N510kEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function N510kEntity:match_set(args)
  if args ~= nil then
...
function N510kEntity:match_get()
...
function N510kEntity:stream(action, args, callopts)
...
function N510kEntity:_run_op(ctx, post_done)
  local utility = self._utility
