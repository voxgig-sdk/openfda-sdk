...
The Lua SDK for the Openfda API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Classification(nil):list(nil, nil)
...
local result, err = client:Openfda(nil):load(
  { id = "test01" }, nil
)
