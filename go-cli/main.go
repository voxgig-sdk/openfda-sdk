// Command Openfda-cli is a CLI + REPL for the Openfda SDK,
...
const prompt = "openfda"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "classification drug drugsfda enforcement event label n510k ndc nsde pma problem shortage substance"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      OPENFDA_APIKEY (api key), OPENFDA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: OPENFDA_APIKEY carries the API key and
	// OPENFDA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("OPENFDA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("OPENFDA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOpenfdaSDK(opts)
