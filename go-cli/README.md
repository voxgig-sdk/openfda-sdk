...
boru-driven command-line client **and** interactive REPL for the Openfda
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/openfda-cli)
make build
...
./openfda-cli list classification
./openfda-cli list drug

# 5. Override the API base URL for a single call
OPENFDA_BASE=https://api.example.com ./openfda-cli list classification

# 6. No arguments -> interactive REPL
./openfda-cli
openfda> list classification
openfda> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/openfda-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/openfda-cli list classification
   ```
...
Run with no arguments to open a REPL (prompt `openfda>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `classification`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/openfda-cli`. |
...
(`./openfda-cli <expr>`) and interactively (the REPL), and expressions compose the
