...
class ClassificationEntity
  ...
  def make
    opts = @_entopts.dup
    ClassificationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = OpenfdaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Classification, Hash] the current Classification data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = OpenfdaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Classification fields)
  def match_get
  ...
  #
  # @param reqmatch [ClassificationListMatch, Hash, nil] match filter (any subset of
  #   Classification fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Classification>, Array] the matching Classification items; raises OpenfdaError on failure
