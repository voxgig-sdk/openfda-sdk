...
module OpenfdaUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = OpenfdaResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OpenfdaResponse.new(fetched)
    else
