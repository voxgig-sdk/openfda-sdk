...
class N510kDirectTest < Minitest::Test
  def test_direct_list_n510k
    setup = n510k_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-n510k", setup[:live] ? "live" : "unit")
    if _should_skip
...
def n510k_direct_setup(mockres)
