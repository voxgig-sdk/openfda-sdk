...
class ClassificationDirectTest < Minitest::Test
  def test_direct_list_classification
    setup = classification_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-classification", setup[:live] ? "live" : "unit")
    if _should_skip
...
def classification_direct_setup(mockres)
