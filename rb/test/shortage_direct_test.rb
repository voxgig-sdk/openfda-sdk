...
class ShortageDirectTest < Minitest::Test
  def test_direct_list_shortage
    setup = shortage_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-shortage", setup[:live] ? "live" : "unit")
    if _should_skip
...
def shortage_direct_setup(mockres)
