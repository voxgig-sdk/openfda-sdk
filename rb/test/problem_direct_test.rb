...
class ProblemDirectTest < Minitest::Test
  def test_direct_list_problem
    setup = problem_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-problem", setup[:live] ? "live" : "unit")
    if _should_skip
...
def problem_direct_setup(mockres)
