...
class SubstanceDirectTest < Minitest::Test
  def test_direct_list_substance
    setup = substance_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-substance", setup[:live] ? "live" : "unit")
    if _should_skip
...
def substance_direct_setup(mockres)
