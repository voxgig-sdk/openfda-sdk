...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = OpenfdaOmni.make_runner(TEST_JSON_FILE, OpenfdaSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : OpenfdaOmni.errify(err)
    end
