...
class DrugsfdaDirectTest < Minitest::Test
  def test_direct_list_drugsfda
    setup = drugsfda_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-drugsfda", setup[:live] ? "live" : "unit")
    if _should_skip
...
def drugsfda_direct_setup(mockres)
