...
class EnforcementDirectTest < Minitest::Test
  def test_direct_list_enforcement
    setup = enforcement_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-enforcement", setup[:live] ? "live" : "unit")
    if _should_skip
...
def enforcement_direct_setup(mockres)
