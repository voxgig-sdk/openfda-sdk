...
{
    private static function utility(): OpenfdaUtility
    {
        return new OpenfdaUtility();
    }
    ...
    private static function ctx(array $over = []): OpenfdaContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new OpenfdaContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new OpenfdaOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof OpenfdaError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new OpenfdaOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = OpenfdaMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new OpenfdaOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = OpenfdaMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new OpenfdaOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = OpenfdaMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenfdaSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = OpenfdaMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new OpenfdaResponse([]),
            'result' => new OpenfdaResult([]),
        ]);
        [, $err] = OpenfdaMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new OpenfdaSpec([]),
            'response' => null,
            'result' => new OpenfdaResult([]),
        ]);
        [, $err] = OpenfdaMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new OpenfdaSpec(['step' => 's']),
            'response' => new OpenfdaResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new OpenfdaResult([]),
        ]);
        [, $err] = OpenfdaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenfdaSpec(['step' => 's']),
            'response' => new OpenfdaResponse(self::resp(200, ['v' => 1])),
            'result' => new OpenfdaResult([]),
        ]);
        [, $err] = OpenfdaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenfdaSpec(['step' => 's']),
            'response' => new OpenfdaResponse(self::resp(200, ['v' => 2])),
            'result' => new OpenfdaResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        OpenfdaMakeResponse::call($ctx);
    ...
    {
        $preset = new OpenfdaResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new OpenfdaSpec([]),
            'response' => new OpenfdaResponse([]),
            'result' => new OpenfdaResult([]),
        ...
        [$got, $err] = OpenfdaMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new OpenfdaResult([])]);
        [, $err] = OpenfdaMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new OpenfdaSpec([]), 'result' => null]);
        [, $err] = OpenfdaMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenfdaSpec(['step' => 's']),
            'result' => new OpenfdaResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new OpenfdaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenfdaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenfdaSpec(['step' => 's']),
            'result' => new OpenfdaResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new OpenfdaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenfdaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenfdaResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new OpenfdaSpec([]),
            'result' => new OpenfdaResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = OpenfdaMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenfdaMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new OpenfdaError('boom', 'boom');
        $utility->fetcher = function (OpenfdaContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenfdaSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = OpenfdaMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (OpenfdaContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = OpenfdaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(OpenfdaResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (OpenfdaContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenfdaSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = OpenfdaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new OpenfdaResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new OpenfdaSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = OpenfdaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = OpenfdaMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = OpenfdaMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new OpenfdaResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, OpenfdaDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new OpenfdaResult(['ok' => false])]);
        $this->expectException(OpenfdaError::class);
        OpenfdaDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new OpenfdaResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', OpenfdaMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        OpenfdaMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new OpenfdaBaseFeature();
        $b = new OpenfdaBaseFeature();
        OpenfdaFeatureAdd::call($ctx, $a);
        OpenfdaFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): OpenfdaBaseFeature
    {
        $f = new OpenfdaBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        OpenfdaFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): OpenfdaContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new OpenfdaSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenfdaPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new OpenfdaResponse(['headers' => null]),
            'result' => new OpenfdaResult([]),
        ]);
        OpenfdaResultHeaders::call($ctx);
