...
class FtCtrl extends OpenfdaControl
{
...
class FtClient
{
    ...
    private OpenfdaUtility $utility;

    public function __construct(OpenfdaUtility $utility, array $options)
    {
    ...
    public function get_utility(): OpenfdaUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OpenfdaContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OpenfdaContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OpenfdaConfig::make_feature($name);
    ...
    private static function buildUrl(OpenfdaSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OpenfdaError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OpenfdaResponse($fetched);
        }
    ...
    private function fail(OpenfdaContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OpenfdaContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
