...
class OpenfdaRetryFeature extends OpenfdaBaseFeature
{
    ...
    public function init(OpenfdaContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (OpenfdaContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(OpenfdaContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(OpenfdaContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
