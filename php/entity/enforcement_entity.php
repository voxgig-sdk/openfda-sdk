...
// Openfda SDK Enforcement entity
...
class EnforcementEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new EnforcementEntity($this->_client, $opts);
    }

    /**
     * @param Enforcement|array $args Enforcement data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = OpenfdaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Enforcement|array The current Enforcement data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Enforcement fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = OpenfdaHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Enforcement fields).
     */
    ...
    /**
     * List Enforcement items matching the given filter.
     *
     * @param EnforcementListMatch|array|null $reqmatch Match filter (any subset
     *   of Enforcement fields) as an assoc-array; EnforcementListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Enforcement[]|array A list of Enforcement items as assoc-arrays at
