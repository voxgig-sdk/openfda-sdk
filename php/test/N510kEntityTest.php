...
class N510kEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = OpenfdaSDK::test(null, null);
        $ent = $testsdk->N510k(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->N510k(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->N510k(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = n510k_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "n510k." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $n510k_ref01_data = null;
        if (count($n510k_ref01_data_raw) > 0) {
...
function n510k_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/n510k/N510kTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["n510k01", "n510k02", "n510k03"] as $k) {
