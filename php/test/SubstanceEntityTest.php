...
class SubstanceEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = OpenfdaSDK::test(null, null);
        $ent = $testsdk->Substance(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Substance(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Substance(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = substance_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "substance." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set OPENFDA_TEST_SUBSTANCE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $substance_ref01_data = null;
        if (count($substance_ref01_data_raw) > 0) {
...
function substance_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/substance/SubstanceTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["substance01", "substance02", "substance03"] as $k) {
    ...
    $entid_env_raw = getenv("OPENFDA_TEST_SUBSTANCE_ENTID");
