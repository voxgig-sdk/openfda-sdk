...
class OpenfdaNetsimFeature extends OpenfdaBaseFeature
{
    ...
    private function _simulate(OpenfdaContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(OpenfdaContext $ctx, array $applied): void
    {
        $client = $this->client;
