...
from openfda_sdk.openfda_types import (
    Enforcement,
    EnforcementListMatch,
)
...
class EnforcementEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return EnforcementEntity(self._client, opts)
    ...
    def match_get(self) -> Enforcement:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Enforcement]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Enforcement().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
