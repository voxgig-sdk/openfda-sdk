...
The Python SDK for the Openfda API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Classification(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Openfda(None).load(
    {"id": "test01"}, None
)
