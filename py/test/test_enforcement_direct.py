...
class TestEnforcementDirect:

    def test_should_direct_list_enforcement(self):
        setup = _enforcement_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-enforcement", "live" if setup["live"] else "unit")
        if _skip:
...
def _enforcement_direct_setup(mockres):
