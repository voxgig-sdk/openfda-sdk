...
class TestNsdeDirect:

    def test_should_direct_list_nsde(self):
        setup = _nsde_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-nsde", "live" if setup["live"] else "unit")
        if _skip:
...
def _nsde_direct_setup(mockres):
