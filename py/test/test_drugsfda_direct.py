...
class TestDrugsfdaDirect:

    def test_should_direct_list_drugsfda(self):
        setup = _drugsfda_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-drugsfda", "live" if setup["live"] else "unit")
        if _skip:
...
def _drugsfda_direct_setup(mockres):
