...
class OpenfdaTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            OpenfdaTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = OpenfdaTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = OpenfdaTestRunner.getenv("OPENFDA_TEST_LIVE")
        override = OpenfdaTestRunner.getenv("OPENFDA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = OpenfdaTestRunner.getenv(key)
        ...
        explain = OpenfdaTestRunner.getenv("OPENFDA_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["OPENFDA_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if OpenfdaTestRunner._test_control is not None:
            return OpenfdaTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                OpenfdaTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            OpenfdaTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = OpenfdaTestRunner.load_test_control()
...
def load_env_local():
    OpenfdaTestRunner.load_env_local()


def env_override(m):
    return OpenfdaTestRunner.env_override(m)


def entity_data(v):
    return OpenfdaTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return OpenfdaTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return OpenfdaTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return OpenfdaTestRunner.load_test_control()


def live_client_options():
    return OpenfdaTestRunner.live_client_options()


def live_delay_ms():
    return OpenfdaTestRunner.live_delay_ms()
