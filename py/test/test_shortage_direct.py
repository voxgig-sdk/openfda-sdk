...
class TestShortageDirect:

    def test_should_direct_list_shortage(self):
        setup = _shortage_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-shortage", "live" if setup["live"] else "unit")
        if _skip:
...
def _shortage_direct_setup(mockres):
