...
class TestSubstanceDirect:

    def test_should_direct_list_substance(self):
        setup = _substance_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-substance", "live" if setup["live"] else "unit")
        if _skip:
...
def _substance_direct_setup(mockres):
