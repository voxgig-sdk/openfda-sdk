...
class TestClassificationDirect:

    def test_should_direct_list_classification(self):
        setup = _classification_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-classification", "live" if setup["live"] else "unit")
        if _skip:
...
def _classification_direct_setup(mockres):
