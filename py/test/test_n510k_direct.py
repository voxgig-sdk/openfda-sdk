...
class TestN510kDirect:

    def test_should_direct_list_n510k(self):
        setup = _n510k_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-n510k", "live" if setup["live"] else "unit")
        if _skip:
...
def _n510k_direct_setup(mockres):
