...
describe('PmaEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'pma.' + op, live)) return
    }
    ...
    let pma_ref01_data = Object.values(setup.data.existing.pma)[0] as any

    // LIST
    const pma_ref01_ent = client.Pma()
    const pma_ref01_match: any = {}

    const pma_ref01_list = (await pma_ref01_ent.list(pma_ref01_match)).map((e: any) => e.data())
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/pma/PmaTestData.json')
