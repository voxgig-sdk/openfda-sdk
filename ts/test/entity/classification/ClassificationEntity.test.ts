...
describe('ClassificationEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'classification.' + op, live)) return
    }
    ...
    let classification_ref01_data = Object.values(setup.data.existing.classification)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/classification/ClassificationTestData.json')
  ...
  let idmap = transform(
    ['classification01','classification02','classification03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
