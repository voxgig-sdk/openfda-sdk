...
describe('NdcEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'ndc.' + op, live)) return
    }
    ...
    let ndc_ref01_data = Object.values(setup.data.existing.ndc)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/ndc/NdcTestData.json')
