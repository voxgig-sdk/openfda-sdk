...
func TestN510kEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.N510k(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.N510k(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		n510kRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.n510k")))
...
func n510kBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("OPENFDA_TEST_N510K_ENTID")
	...
	idmapResolved := core.ToMapAny(env["OPENFDA_TEST_N510K_ENTID"])
