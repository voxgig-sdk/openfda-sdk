...
import (
	...
	omni "github.com/voxgig-sdk/openfda-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OpenfdaError); is {
				out := map[string]any{
					"name":    "OpenfdaError",
...
func omniCtx(arg any, client *sdk.OpenfdaSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OpenfdaSDK); is {
			client = live
